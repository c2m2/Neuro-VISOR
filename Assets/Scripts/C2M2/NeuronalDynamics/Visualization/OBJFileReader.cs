...
namespace C2M2.NeuronalDynamics.Visualization
{
    ...
    {
        public static Mesh ReadFile(in string objFilePath)
        {
            ...
            if (!File.Exists(objFilePath)) { throw new System.Exception("Could not find file " + objFilePath); }
            StreamReader reader = new StreamReader(objFilePath);
            // Get file name to copy to mesh name
            string[] fileSplit = objFilePath.Split(Path.DirectorySeparatorChar);
